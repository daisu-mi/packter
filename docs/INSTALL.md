...
```sh
make              # broker(release) + agent をまとめてビルド（※一般ユーザで）
make broker       # ブローカーだけ
make agent        # エージェントだけ（必要なら autogen→configure も実行）
make check        # 両方のテストを実行
sudo make install # ビルド済みファイルを PREFIX 配下へコピー（既定 /usr/local/packter）
make agent CONFIGURE_FLAGS=--with-geoip   # 例: PACKTEARTH(GeoIP) 有効でビルド
```

> **ビルドは一般ユーザ、インストールだけ root**。`make install` は**ビルドしない**
> （コピーのみ）。先に `make`（自分の権限）でビルドし、その後 `sudo make install`。
> こうしないと `cargo`/`cc` が root で走り、`target/` が root 所有になったり root の
> cargo キャッシュを汚す。未ビルドのまま `sudo make install` するとエラーで止まり、
> 先に `make` するよう促す。
